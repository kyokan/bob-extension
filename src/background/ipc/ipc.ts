  ...
  const handler = (event, data) => {
    ...
    if (!data.id) {
      ...
      return event.sender.send(
        SIGIL,
        makeError(-32600, "id not provided", null)
      );
    }
    ...
    if (!method) {
      ...
      return event.sender.send(
        SIGIL,
        makeError(-32601, "method not found", data.id)
      );
    }
    ...
    const cb = (err, res) => {
      if (err) {
        ...
        return event.sender.send(
          SIGIL,
          makeError(err.code || -1, err.message, data.id)
        );
      }
      log(
        "Sending IPC method response.",
        sanitizeData(data, method),
        sanitizeRes(res, method)
      );
    ...
    if (maybePromise.then) {
      maybePromise.then((res) => cb(null, res)).catch((err) => cb(err));
    }
  };
